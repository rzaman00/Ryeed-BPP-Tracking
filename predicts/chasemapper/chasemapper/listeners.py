...
def fix_datetime(datetime_str, local_dt_str=None):
    ...
    if local_dt_str is None:
        # Naive UTC: passed as `default=` to dateutil.parse() below, which expects
        # a naive datetime when parsing a naive HH:MM:SS input. Mixing aware/naive
        # via the default would change the type of the returned _telem_dt.
