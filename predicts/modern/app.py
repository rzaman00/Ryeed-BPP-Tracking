...
FORECAST_COMPARISON_CACHE_TTL_S = 15 * 60.0
_forecast_comparison_cache: dict[tuple[Any, ...], tuple[float, dict[str, Any]]] = {}
FORECAST_MODELS = (
    ("best_match", "Open-Meteo Best Match", None, "#2563eb"),
    ("gfs", "NOAA GFS", "gfs_seamless", "#dc2626"),
    ("ecmwf", "ECMWF IFS", "ecmwf_ifs025", "#16a34a"),
    ("icon", "DWD ICON", "icon_seamless", "#9333ea"),
)
FORECAST_PRESSURE_LEVELS_HPA = (1000, 925, 850, 700, 600, 500, 400, 300, 250, 200, 150, 100, 70, 50, 30)
FAA_AUTO_REFRESH_INTERVAL_S = 15 * 60
_faa_refresh_task: asyncio.Task[Any] | None = None


async def _faa_refresh_loop() -> None:
    """Keep FAA caches current without delaying application startup."""
    while True:
        await asyncio.sleep(FAA_AUTO_REFRESH_INTERVAL_S)
        try:
            await refresh_faa_datasets(("controlled", "class_e", "sua", "tfr"), force=False)
        except Exception:
            # Each operational_airspace call already preserves stale cache data.
            # The loop must remain alive through a temporary FAA outage.
            pass


@asynccontextmanager
async def app_lifespan(_: FastAPI):
    global _faa_refresh_task
    _faa_refresh_task = asyncio.create_task(_faa_refresh_loop(), name="bpp-faa-auto-refresh")
    try:
        yield
    finally:
        _faa_refresh_task.cancel()
        with suppress(asyncio.CancelledError):
            await _faa_refresh_task
        _faa_refresh_task = None
...
class AirspaceRefreshRequest(BaseModel):
    datasets: list[Literal["controlled", "class_e", "sua", "tfr"]] = Field(
...
def _nearest_hour_index(times: list[Any], requested: datetime) -> tuple[int, datetime]:
    parsed: list[tuple[int, datetime]] = []
    for source_index, raw_time in enumerate(times):
        try:
            candidate = datetime.fromisoformat(str(raw_time))
        except ValueError:
            continue
        ...
        parsed.append((source_index, candidate))
    if not parsed:
        raise ExternalServiceError("forecast model returned unreadable times")
    return min(parsed, key=lambda item: abs((item[1] - requested).total_seconds()))


def _wind_components(speed_mph: float, direction_deg: float) -> tuple[float, float]:
    """Convert meteorological wind-from direction into east/north m/s."""
    speed_ms = max(0.0, float(speed_mph)) * 0.44704
    radians = math.radians(float(direction_deg) % 360.0)
    return -speed_ms * math.sin(radians), -speed_ms * math.cos(radians)


def _profile_components(profile: list[dict[str, Any]], altitude_m: float) -> tuple[float, float]:
    valid = [
        row for row in profile
        if row.get("wind_speed_mph") is not None and row.get("wind_direction_deg") is not None
    ]
    if not valid:
        raise ExternalServiceError("forecast model returned no winds-aloft profile")
    valid.sort(key=lambda row: float(row["altitude_m"]))
    if altitude_m <= float(valid[0]["altitude_m"]):
        return _wind_components(valid[0]["wind_speed_mph"], valid[0]["wind_direction_deg"])
    if altitude_m >= float(valid[-1]["altitude_m"]):
        return _wind_components(valid[-1]["wind_speed_mph"], valid[-1]["wind_direction_deg"])
    for lower, upper in zip(valid, valid[1:]):
        low_alt, high_alt = float(lower["altitude_m"]), float(upper["altitude_m"])
        if low_alt <= altitude_m <= high_alt:
            fraction = (altitude_m - low_alt) / max(1.0, high_alt - low_alt)
            low_u, low_v = _wind_components(lower["wind_speed_mph"], lower["wind_direction_deg"])
            high_u, high_v = _wind_components(upper["wind_speed_mph"], upper["wind_direction_deg"])
            return low_u + (high_u - low_u) * fraction, low_v + (high_v - low_v) * fraction
    return _wind_components(valid[-1]["wind_speed_mph"], valid[-1]["wind_direction_deg"])


def estimate_model_trajectory(req: PredictRequest, profile: list[dict[str, Any]]) -> dict[str, Any]:
    """Build a screening trajectory from one model's launch-column winds.

    Tawhiri remains the authoritative spatial prediction. These estimates use the
    same launch parameters against each model's vertical wind column so operators
    can see forecast disagreement before launch.
    """
    latitude = float(req.launch.latitude)
    longitude = float(req.launch.longitude)
    ground_altitude = max(0.0, float(req.launch.altitude_m or 0.0))
    altitude = ground_altitude
    elapsed = 0.0
    step_s = 60.0
    coordinates: list[list[float]] = [[longitude, latitude, altitude]]

    def advance(seconds: float, vertical_rate_ms: float) -> None:
        nonlocal latitude, longitude, altitude, elapsed
        u_ms, v_ms = _profile_components(profile, altitude)
        latitude, longitude = _advect(latitude, longitude, u_ms, v_ms, seconds)
        altitude = max(ground_altitude, altitude + vertical_rate_ms * seconds)
        elapsed += seconds
        coordinates.append([longitude, latitude, altitude])

    top_altitude = float(req.burst_altitude_m if req.mode == "burst" else req.float_altitude_m)
    for _ in range(3000):
        if altitude >= top_altitude - 0.1:
            break
        seconds = min(step_s, (top_altitude - altitude) / req.ascent_rate_ms)
        advance(seconds, req.ascent_rate_ms)
    else:
        raise ExternalServiceError("model trajectory ascent exceeded its safety limit")

    if req.mode == "float":
        remaining = float(req.float_duration_min) * 60.0
        while remaining > 0:
            seconds = min(step_s, remaining)
            advance(seconds, req.float_ascent_rate_ms)
            remaining -= seconds

    for _ in range(3000):
        if altitude <= ground_altitude + 0.1:
            break
        descent_rate = req.descent_rate_ms * math.exp(max(0.0, altitude - ground_altitude) / 17_000.0)
        seconds = min(step_s, (altitude - ground_altitude) / max(0.01, descent_rate))
        advance(seconds, -descent_rate)
    else:
        raise ExternalServiceError("model trajectory descent exceeded its safety limit")

    ground_distance = sum(
        haversine_m((a[1], a[0]), (b[1], b[0])) for a, b in zip(coordinates, coordinates[1:])
    )
    return {
        "landing": {"latitude": latitude, "longitude": longitude},
        "flight_duration_s": round(elapsed),
        "ground_distance_m": ground_distance,
        "trajectory": coordinates,
        "method": "launch-column winds screening estimate",
    }


async def fetch_forecast_model(
    req: PredictRequest,
    model_id: str,
    model_name: str,
    api_model: str | None,
    ...
    for pressure in FORECAST_PRESSURE_LEVELS_HPA:
        hourly_fields.extend((f"wind_speed_{pressure}hPa", f"wind_direction_{pressure}hPa"))
    params: dict[str, Any] = {
        "latitude": req.launch.latitude,
        "longitude": req.launch.longitude,
    ...
    hourly = payload.get("hourly") or {}
    times = hourly.get("time") or []
    idx, selected_time = _nearest_hour_index(times, _as_utc(req.launch_datetime))
    ...
    for pressure in FORECAST_PRESSURE_LEVELS_HPA:
        speed = at(f"wind_speed_{pressure}hPa")
        direction = at(f"wind_direction_{pressure}hPa")
        if speed is None or direction is None:
            continue
        altitude_m = float(NCEP_PRESSURE_HEIGHT_M[float(pressure)])
        profile.append({
            "pressure_hpa": pressure,
            "altitude_m": altitude_m,
            "altitude_ft": altitude_m * 3.28084,
            "wind_speed_mph": float(speed),
            "wind_direction_deg": float(direction),
        })
    if len(profile) < 4:
        raise ExternalServiceError(f"{model_name} returned an incomplete winds-aloft profile")
    rain_in = float(at("rain") or 0.0)
    precipitation_in = float(at("precipitation") or 0.0)
    surface = {
    ...
    return {
        ...
        "api_model": api_model or "best_match",
        "color": color,
        "datetime": utc_iso(selected_time),
        "surface": surface,
        "profile": profile,
        "estimate": estimate_model_trajectory(req, profile),
    }


async def forecast_model_comparison(req: PredictRequest) -> dict[str, Any]:
    validate_launch_window(req.launch_datetime)
    key = (
        round(req.launch.latitude, 3), round(req.launch.longitude, 3),
        _as_utc(req.launch_datetime).strftime("%Y-%m-%dT%H"), req.mode,
        round(req.ascent_rate_ms, 2), round(req.descent_rate_ms, 2),
        round(req.burst_altitude_m, 0), round(req.float_altitude_m, 0),
        round(req.float_ascent_rate_ms, 2), round(req.float_duration_min, 1),
    )
    cached = _forecast_comparison_cache.get(key)
    if cached and time.monotonic() - cached[0] < FORECAST_COMPARISON_CACHE_TTL_S:
        return dict(cached[1])

    async with httpx.AsyncClient(timeout=httpx.Timeout(35.0, connect=8.0), follow_redirects=True) as client:
        async def one(model: tuple[str, str, str | None, str]) -> dict[str, Any]:
            model_id, model_name, api_model, color = model
            try:
                result = await fetch_forecast_model(req, model_id, model_name, api_model, color, client)
                return {"ok": True, **result}
            except Exception as exc:
                return {
                    "ok": False, "model_id": model_id, "model_name": model_name,
                    "api_model": api_model or "best_match", "color": color, "error": str(exc),
                }

        models = await asyncio.gather(*(one(model) for model in FORECAST_MODELS))

    available = [model for model in models if model.get("ok") and model.get("estimate", {}).get("landing")]
    agreement: dict[str, Any] = {
        "level": "unavailable", "max_landing_spread_m": None,
        "gust_spread_mph": None, "conservative_model_id": None,
    }
    if available:
        landings = [model["estimate"]["landing"] for model in available]
        consensus = {
            "latitude": sum(point["latitude"] for point in landings) / len(landings),
            "longitude": sum(point["longitude"] for point in landings) / len(landings),
        }
        max_spread = 0.0
        for index, first in enumerate(landings):
            for second in landings[index + 1:]:
                max_spread = max(max_spread, haversine_m(
                    (first["latitude"], first["longitude"]),
                    (second["latitude"], second["longitude"]),
                ))
        for model in available:
            landing = model["estimate"]["landing"]
            model["distance_from_consensus_m"] = haversine_m(
                (landing["latitude"], landing["longitude"]),
                (consensus["latitude"], consensus["longitude"]),
            )
        gust_values = [
            float(model["surface"]["wind_gust_mph"])
            for model in available if model.get("surface", {}).get("wind_gust_mph") is not None
        ]
        gust_spread = max(gust_values) - min(gust_values) if len(gust_values) > 1 else 0.0
        conservative = max(available, key=lambda model: (
            float(model.get("surface", {}).get("wind_gust_mph") or 0.0),
            float(model.get("estimate", {}).get("ground_distance_m") or 0.0),
        ))
        level = "high" if len(available) >= 3 and max_spread <= 40_000 else "moderate" if len(available) >= 2 and max_spread <= 120_000 else "low"
        agreement = {
            "level": level,
            "model_count": len(available),
            "consensus_landing": consensus,
            "max_landing_spread_m": max_spread,
            "gust_spread_mph": gust_spread,
            "conservative_model_id": conservative["model_id"],
        }

    result = {
        "launch": req.launch.model_dump(),
        "launch_datetime": utc_iso(req.launch_datetime),
        "models": models,
        "agreement": agreement,
        "primary_prediction": "Tawhiri remains the primary BPP trajectory",
        "comparison_method": "Model tracks are launch-column winds screening estimates",
        "generated_at": utc_iso(datetime.now(timezone.utc)),
    }
    _forecast_comparison_cache[key] = (time.monotonic(), result)
    if len(_forecast_comparison_cache) > 24:
        oldest = min(_forecast_comparison_cache, key=lambda item: _forecast_comparison_cache[item][0])
        _forecast_comparison_cache.pop(oldest, None)
    return dict(result)
...
def airspace_cache_status(dataset: str) -> dict[str, Any]:
    cache_path = AIRSPACE_CACHE_DIR / f"{dataset}.geojson"
    try:
        ...
        return {
            "dataset": dataset,
            "updated_at": utc_iso(modified),
            "age_seconds": round(age_seconds),
            "ttl_seconds": AIRSPACE_TTLS[dataset],
            "stale": age_seconds >= AIRSPACE_TTLS[dataset],
            "available": True,
        }
    except OSError:
        return {
            "dataset": dataset, "updated_at": None, "age_seconds": None,
            "ttl_seconds": AIRSPACE_TTLS[dataset], "stale": True, "available": False,
        }


async def refresh_faa_datasets(datasets: tuple[str, ...] | list[str], force: bool) -> dict[str, Any]:
    unique = tuple(dict.fromkeys(dataset for dataset in datasets if dataset in AIRSPACE_TTLS))

    async def one(dataset: str) -> dict[str, Any]:
        data, warning, source = await operational_airspace(dataset, force=force)
        return {
            **airspace_cache_status(dataset),
            "source": source,
            "warning": warning,
            "count": len(data.get("features", [])),
        }

    results = await asyncio.gather(*(one(dataset) for dataset in unique))
    return {
        "datasets": results,
        "automatic_refresh_seconds": FAA_AUTO_REFRESH_INTERVAL_S,
...
async def health():
    return {
        ...
        "launch_weather": "Open-Meteo multi-model forecast and winds aloft",
        "faa_refresh": f"automatic every {FAA_AUTO_REFRESH_INTERVAL_S // 60} minutes with disk cache",
    }
...
async def config():
    return {
        ...
        "weather": True,
        "forecast_models": [name for _, name, _, _ in FORECAST_MODELS],
        "faa_auto_refresh_minutes": FAA_AUTO_REFRESH_INTERVAL_S // 60,
    }
...
async def airspace_status():
    return {
        "datasets": [airspace_cache_status(dataset) for dataset in ("controlled", "class_e", "sua", "tfr")],
        "automatic_refresh_seconds": FAA_AUTO_REFRESH_INTERVAL_S,
        "checked_at": utc_iso(datetime.now(timezone.utc)),
    }
...
async def airspace_refresh(req: AirspaceRefreshRequest):
    return await refresh_faa_datasets(req.datasets, force=True)
...
@app.post("/api/forecast-comparison")
async def forecast_comparison(req: PredictRequest):
    try:
        return await forecast_model_comparison(req)
    except ExternalServiceError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
