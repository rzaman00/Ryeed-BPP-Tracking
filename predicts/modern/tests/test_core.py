...
def test_v30_build_contract_and_docs():
    ...
    assert '3.7.2' in (base.parent/'VERSION.txt').read_text(encoding='utf-8')
...
def test_v37_complete_chasemapper_and_basic_safety_contract():
    ...
    assert 'href="/live-chase"' in html
    waiting=(base/'static'/'live_chase.html').read_text(encoding='utf-8')
    assert '/api/live-chase/status' in waiting
    assert 'location.replace(data.url)' in waiting
    ...
    assert "setAppView('live')" not in js
    assert '@app.get("/api/live")' not in app_source
    assert '@app.post("/api/live/predict")' not in app_source
...
def test_v372_launcher_starts_chase_in_background_and_reports_status():
    base=Path(__file__).resolve().parents[1]
    launcher=(base/'run_latest.py').read_text(encoding='utf-8')
    assert 'threading.Thread(target=start_live_chase' in launcher
    assert 'docker_desktop_executable' in launcher
    assert 'Docker Desktop.exe' in launcher
    assert 'write_chase_status' in launcher
