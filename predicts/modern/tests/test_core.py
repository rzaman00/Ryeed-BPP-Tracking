...
def test_model_screening_trajectory_and_comparison(monkeypatch):
    import asyncio
    import app as appmod
    ...
    req = appmod.PredictRequest(
        mode="burst",
        launch=appmod.LaunchPoint(name="Test", latitude=39.5, longitude=-77.5, altitude_m=150),
        launch_datetime=launch_time,
        ascent_rate_ms=5.5,
        descent_rate_ms=9,
        burst_altitude_m=12000,
    )
    profile = [
        {"altitude_m": altitude, "wind_speed_mph": 12 + index, "wind_direction_deg": 270}
        for index, altitude in enumerate((110, 1450, 3010, 5570, 9160, 13610))
    ]
    estimate = appmod.estimate_model_trajectory(req, profile)
    assert estimate["flight_duration_s"] > 0
    assert estimate["ground_distance_m"] > 0
    assert estimate["landing"]["longitude"] > req.launch.longitude
    assert len(estimate["trajectory"]) > 20

    offsets = {"best_match": 0.0, "gfs": 0.04, "ecmwf": -0.03, "icon": 0.02}
    gusts = {"best_match": 8.0, "gfs": 14.0, "ecmwf": 10.0, "icon": 12.0}

    async def fake_model(request, model_id, model_name, api_model, color, client):
        offset = offsets[model_id]
        return {
            "model_id": model_id, "model_name": model_name, "api_model": api_model or "best_match",
            "color": color, "datetime": appmod.utc_iso(launch_time),
            "surface": {"wind_gust_mph": gusts[model_id]}, "profile": profile,
            "estimate": {
                "landing": {"latitude": 39.5 + offset, "longitude": -77.3 + offset},
                "flight_duration_s": 4000, "ground_distance_m": 25000 + abs(offset) * 10000,
                "trajectory": [[-77.5, 39.5, 150], [-77.3 + offset, 39.5 + offset, 150]],
            },
        }

    appmod._forecast_comparison_cache.clear()
    monkeypatch.setattr(appmod, "fetch_forecast_model", fake_model)
    class FakeClient:
    ...
    result = asyncio.run(appmod.forecast_model_comparison(req))
    assert len(result["models"]) == 4
    assert all(model["ok"] for model in result["models"])
    assert result["agreement"]["level"] == "high"
    assert result["agreement"]["conservative_model_id"] == "gfs"
    assert result["agreement"]["gust_spread_mph"] == 6.0
    assert all(model["distance_from_consensus_m"] >= 0 for model in result["models"])


def test_faa_status_and_refresh_report_cache_freshness(monkeypatch, tmp_path):
    import asyncio
    import app as appmod

    cache_dir = tmp_path / "airspace"
    cache_dir.mkdir()
    (cache_dir / "tfr.geojson").write_text('{"type":"FeatureCollection","features":[]}', encoding="utf-8")
    monkeypatch.setattr(appmod, "AIRSPACE_CACHE_DIR", cache_dir)
    status = appmod.airspace_cache_status("tfr")
    assert status["available"] is True
    assert status["stale"] is False
    assert status["age_seconds"] >= 0

    async def fake_airspace(dataset, force=False):
        return {"type": "FeatureCollection", "features": [{"id": dataset}]}, None, "FAA live"

    monkeypatch.setattr(appmod, "operational_airspace", fake_airspace)
    refreshed = asyncio.run(appmod.refresh_faa_datasets(["tfr", "tfr"], force=True))
    assert refreshed["automatic_refresh_seconds"] == 900
    assert len(refreshed["datasets"]) == 1
    assert refreshed["datasets"][0]["count"] == 1
    assert refreshed["datasets"][0]["source"] == "FAA live"
...
def test_health_and_config_report_final_build():
    ...
    assert set(c["forecast_models"]) == {"Open-Meteo Best Match", "NOAA GFS", "ECMWF IFS", "DWD ICON"}
    assert c["faa_auto_refresh_minutes"] == 15
...
def test_v38_forecast_faa_safety_and_prediction_only_contract():
    base=Path(__file__).resolve().parents[1]
    html=(base/'static'/'index.html').read_text(encoding='utf-8')
    js=(base/'static'/'app.js').read_text(encoding='utf-8')
    ...
    assert 'live-chase' not in html
    assert not (base/'static'/'live_chase.html').exists()
    ...
    for item in ['safetyRulesTab','safetyRulesView','saveSafetyRules','resetSafetyRules','forecastPanel','forecastModelTableBody','windsAloftChart','faaStatusChip','refreshFaaData']:
        assert f'id="{item}"' in html
        assert item in js
    for endpoint in ['/api/forecast-comparison','/api/airspace-status','/api/airspace-refresh']:
        assert endpoint in app_source or endpoint in js
    for token in ['live-chase', '/api/live', 'ChaseMapper', 'start_live_chase']:
...
def test_v371_startup_checks_only_modern_predicts_dependencies():
    base=Path(__file__).resolve().parents[1]
    startup_files=[
        base/'setup_windows.bat', base/'setup_windows.ps1',
        base/'setup_unix.sh', base/'run_windows.bat',
    ]
    for path in startup_files:
        source=path.read_text(encoding='utf-8')
        assert 'aprslib' not in source
        assert 'import fastapi' in source


def test_v38_launcher_starts_only_prediction_server():
    base=Path(__file__).resolve().parents[1]
    launcher=(base/'run_latest.py').read_text(encoding='utf-8')
    assert 'subprocess.Popen([sys.executable, str(ROOT / "app.py")]' in launcher
    for token in ['start_live_chase', 'docker', 'ChaseMapper', 'threading.Thread']:
