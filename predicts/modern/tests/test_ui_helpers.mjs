...
import {
  ...
  deriveCityLabel,
  evaluateReadiness,
  formatSweepParameter,
  normalizeSafetyRules,
  sortReadinessRows,
} from '../static/ui_helpers.mjs';
...
assert.equal(customRules.forecastNoGoMinutes,DEFAULT_SAFETY_RULES.forecastNoGoMinutes);
assert.equal(evaluateReadiness({wind_gust_mph:7,rain:false,precipitation_in:0},safeOptimal,90,customRules).status,'go');
assert.equal(evaluateReadiness({wind_gust_mph:18,rain:false,precipitation_in:0},safeOptimal,10,customRules).status,'caution');
assert.equal(evaluateReadiness({wind_gust_mph:21,rain:false,precipitation_in:0},safeOptimal,10,customRules).status,'no-go');
